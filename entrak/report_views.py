...
def __generate_report_data(systems, report_type, start_timestamp, end_timestamp):
    ...
    for source_id, readings in source_readings.items():
        ...
        target_info['last_year_this_month']['enegry'] += last_year_this_month_enegry
        target_info['last_year_this_month']['money'] += calculation.transform_reading(
            money_unit_code,
...
@permission_required()
def download_summary_report_view(request, system_code, start_timestamp, end_timestamp, report_type):
    if request.is_secure():
        domain_url = "https://" + request.META['HTTP_HOST']
    else:
        domain_url = "http://" + request.META['HTTP_HOST']
